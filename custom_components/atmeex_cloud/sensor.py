...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities):
    ...
    _LOGGER.info(f"Setting up sensor entities for {device_count} device(s)")
    
    if device_count == 0:
        _LOGGER.warning("No devices in coordinator when setting up sensor entities. Entities will be empty.")
    ...
    for device in coordinator.devices:
        ...
        # Создаем только sensors которые доступны через settings (не зависят от condition)
        # CO2, Temperature, Humidity удалены - они доступны только когда устройство онлайн через WebSocket
