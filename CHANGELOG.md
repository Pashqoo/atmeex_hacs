...
### Удалено
- **Даткии, зависящие от `condition`** (недоступны через REST API):
  - **CO2 sensor** - концентрация CO2 в ppm
  - **Temperature sensor** - температура в комнате
  - **Temperature In sensor** - температура на входе
  - **Humidity sensor** - влажность в комнате
  - **No Water binary sensor** - индикатор отсутствия воды

### Обоснование
Данные из `condition` доступны только когда устройство онлайн через WebSocket, а не через REST API. В REST API `condition` всегда `null` когда устройство показывает `online: false`. Эти данные можно получить только если устройство активно подключено через WebSocket (например, если открыто мобильное приложение Atmeex).

### Оставлено
- Все датчики из `settings` (доступны всегда):
  - **Humidity Target** - целевая влажность
  - **Fan Speed** - скорость вентилятора
  - **Damper Position** - положение заслонки
  - **Firmware Version** - версия прошивки
- Все entities управления (climate, fan, switches, select, number)
- **Online binary sensor** - статус онлайн (из `model.online`)

## [0.4.0] - 2025-01-17

### Добавлено
- **Binary Sensors (Двоичные датчики)**:
  - **No Water** - индикатор отсутствия воды
  - **Online** - статус подключения к облаку
  
- **Select Entity (Выбор режима)**:
  - **Damper Position** - управление положением заслонки (открыта/смешанный/закрыта)
  - Используется для пассивного проветривания
  
- **Number Entity (Числовое значение)**:
  - **Humidity Target** - установка целевой влажности (0-100%)
...
- **Дополнительные Sensors (Датчики)**:
  - **Humidity Target** - текущая целевая влажность (%)
  - **Fan Speed** - текущая скорость вентилятора (1-7)
  - **Damper Position** - текущее положение заслонки (текст)
  - **Firmware Version** - версия прошивки устройства

### Улучшено
- Полная реализация всех доступных функций API Atmeex
- Все entities имеют уникальные ID для настройки через UI
- Правильная группировка entities по устройствам
